...
def create_tensors_sentences(
    ...
    dtype = dtype if dtype is not None else activations[0].dtype
    
    X = []
    y = []
    for activation, target in zip(activations, sentences['target']):
        if not np.isnan(activation).any():
            X.append(activation)
            y.append(target)

    # Get unique labels as a dictionary of mappings
    if mappings is not None:
        label2idx, idx2label, src2idx, idx2src = mappings
        # Add encoding for missing labels
        for label in set(y):
            if label not in label2idx.keys():
                idx = len(label2idx)
                label2idx[label] = idx
                idx2label[idx] = label
        # Encode labels using existing mappings
        y = [label2idx[label] for label in y if label in label2idx]
        # Ensure that the labels are consistent with the mappings
        ...
        mappings = (label2idx, idx2label, None, None)
    
    X = np.array(X, dtype=dtype).reshape(-1, activations[0].shape[1])
    y = np.array(y, dtype=int)

    return X, y, mappings


def create_tensors_tokens(
    tokens,
    activations,
    task_specific_tag,
    mappings=None,
    ...
    if task_type == "classification":
        ...
        y = np.zeros((num_tokens,), dtype=float)
