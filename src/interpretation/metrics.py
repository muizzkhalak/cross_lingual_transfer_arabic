...
def f1(preds, labels, average="binary"):
    """
    F-Score or F1 score.
...
def accuracy_and_f1(preds, labels, average="binary"):
    """
    ...
    f1_s = f1_score(y_true=labels, y_pred=preds, average=average)
    return (acc + f1_s) / 2
...
def compute_score(preds, labels, metric, average='binary'):
    """
    ...
    if metric == "accuracy":
        ...
        return f1(preds, labels, average=average)
    elif metric == "accuracy_and_f1":
        return accuracy_and_f1(preds, labels, average=average)
    elif metric == "pearson":
...
def bootstrap_ci(metric_fn, y_pred: np.ndarray, y_true: np.ndarray, n_boot: int = 1000, alpha: float = 0.05, seed: int = 42):
    rng = np.random.default_rng(seed)
    n = len(y_true)
    stats = []
    for _ in range(n_boot):
        idx = rng.integers(0, n, n)
        stats.append(metric_fn(y_pred[idx], y_true[idx]))
    lo = float(np.quantile(stats, alpha/2))
    hi = float(np.quantile(stats, 1 - alpha/2))
    return lo, hi
    

def summarize(pred, labels):
    acc = accuracy(pred, labels); f1m = f1(pred, labels, average="macro")
    acc_lo, acc_hi = bootstrap_ci(accuracy, pred, labels)
    f1_lo, f1_hi = bootstrap_ci(lambda a,b: f1(a,b,average="macro"), pred, labels)
