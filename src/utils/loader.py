    ...
    for idx, activation in enumerate(activations):
        if activation.shape[0] == len(tokens["source"][idx]) and (
            ...
            if activation.shape[0] < len(tokens["source"][idx]) and activation.shape[0] < len(tokens["target"][idx]):
                tokens["source"][idx] = tokens["source"][idx][:activation.shape[0]]
                tokens["target"][idx] = tokens["target"][idx][:activation.shape[0]]
            else:
                invalid_activation_idx.append(idx)
...
def load_data_sentence(
        ...
        return {'source' : sentences, 'target': labels}
